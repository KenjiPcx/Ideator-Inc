...
def create_workflow(
    ...
    **kwargs
    ...
    reporter = create_reporter(chat_history=chat_history, email=email, session_id=session_id)

    workflow = FinancialReportWorkflow(timeout=360, chat_history=chat_history, session_id=session_id)
...
class FinancialReportWorkflow(Workflow):
    def __init__(
        self, timeout: int = 360, chat_history: Optional[List[ChatMessage]] = None, session_id: str = ""
    ):
